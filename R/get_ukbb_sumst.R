...
retrieve_10kloc = function(hl, folder, cache) {
    #
...
#' @param simple logical(1) avoids basilisk if TRUE
...
#' ss$count()
#' @export
get_ukbb_sumstat_10kloci_mt <- function(hl, folder = tempdir(), cache = BiocFileCache::BiocFileCache(), timeout.ukbb = 3600, simple=FALSE) {

  if (simple) {
     return(retrieve_10kloc(hl, folder, cache))
  }
...
#    mt <- hl$read_matrix_table(paste0(folder, "/ukbb_sumstat_10k_loci.mt"))
#    mt
    retrieve_10kloc(hl, folder, cache) 
