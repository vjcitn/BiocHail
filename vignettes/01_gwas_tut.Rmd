...
the vignette).
```{python abc, eval=ISNT_PKGDOWN}
r.mt.rows().select().show(5) # python chunk!
```

The sample IDs:
```{python ch2, eval=ISNT_PKGDOWN}
...
```{r getsup, eval=ISNT_PKGDOWN}
mt$aggregate_cols(hl$agg$counter(mt$pheno$SuperPopulation))
```

Then statistics on caffeine consumption:
```{r lkcaf, eval=ISNT_PKGDOWN}
...
```{r update, eval=ISNT_PKGDOWN}
mt <- hl$sample_qc(mt)
```{python ch6, eval=ISNT_PKGDOWN}
r.mt.col.describe()  # python!
```

The call rate histogram is given by:
```{r lkcr, eval=ISNT_PKGDOWN}
...
```{python dofilt, eval=ISNT_PKGDOWN}
# python chunk!
...
```{python gtfilt, eval=ISNT_PKGDOWN}
...
```{r addvarqc, eval=ISNT_PKGDOWN}
mt = hl$variant_qc(mt)
```
```{python lkvarqc, eval=ISNT_PKGDOWN}
...
```{python dofilts, eval=ISNT_PKGDOWN}
...
```{python naivet, eval=ISNT_PKGDOWN}
...
```{python dopca, eval=ISNT_PKGDOWN}
...
```{python redog, eval=ISNT_PKGDOWN}
